...
contract NodeManagerTest is Test {
    ...
    function setUp() public {
        ...
        token.transfer(cp1, 30000 * 10 ** 18);
        token.transfer(cp2, 30000 * 10 ** 18);
    ...
    function testGetAllComputePools() public {
        // Initially no pools
        uint256[] memory pools = nodeManager.getAllComputePools();
        ...
        nodeManager.registerNode(rating1);
        nodeManager.registerNode(rating2);

        // Check pools
        pools = nodeManager.getAllComputePools();
        assertEq(pools.length, 2, "Should have 2 pools");
        assertTrue(pools[0] == rating1 || pools[1] == rating1, "Should contain rating1");
        assertTrue(pools[0] == rating2 || pools[1] == rating2, "Should contain rating2");

        vm.stopPrank();
    }

    function testComputePoolsUpdateOnUnregister() public {
        vm.startPrank(cp1);
        ...
        uint256 nodeId1 = nodeManager.registerNode(rating);
        uint256 nodeId2 = nodeManager.registerNode(rating);

        // Check initial state
        uint256[] memory pools = nodeManager.getAllComputePools();
        assertEq(pools.length, 1, "Should have 1 pool");
        assertEq(pools[0], rating, "Pool should match rating");

        // Unregister one node
        nodeManager.unregisterNode(nodeId1);
        pools = nodeManager.getAllComputePools();
        assertEq(pools.length, 1, "Should still have 1 pool");

        // Unregister second node
        nodeManager.unregisterNode(nodeId2);
        pools = nodeManager.getAllComputePools();
        assertEq(pools.length, 0, "Should have no pools after all nodes unregistered");
